async function load(){
     ...
     document.querySelector('#type1').src = './assets/' + json.types[0].type.name + '.png';
     document.querySelector('#type2').src = json.types.length > 1 ?
         './assets/' + json.types[1].type.name + '.png' : './assets/default.png';
     document.querySelector('#id').innerHTML = 'Nº ' + json.id;
     
     document.querySelector('#name').innerHTML = json.name[0].toUpperCase() + json.name.slice(1);
